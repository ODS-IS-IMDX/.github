...
| データ登録 | 標準仕様3Dシェープファイルを一時テーブルに登録する機能。|
| 品質チェック | 設備データに利用させている属性コード値と属性コード値を管理するマスタの値を比較し、不一致コード及びフィーチャの情報をログ出力する機能。 |
| データ公開 | 履歴管理からTBL出力（設備データ管理マスタテーブル→3D用最終断面テーブル）までをラップする機能。 |
| 履歴管理 | 設備データ管理マスタテーブルから設備データのダンプファイルを取得し、<br>履歴管理用ストレージにアップロードする機能。 |
| 差分管理 | 一時テーブルと設備データ管理マスタテーブルとの差分を抽出し、差分管理テーブルに登録する機能。 |
| TBL出力<br>（一時テーブル→設備データ管理マスタテーブル） | 一時テーブルから取得したデータに管理用ID等の属性を追加し、設備データ管理マスタテーブルに登録する。その後、一時テーブルのデータは削除する機能。 |
| TBL出力<br>（設備データ管理マスタテーブル→2D用最終断面テーブル） | 設備データ管理マスタテーブルから設備データを取得し、<br>必要に応じてデータの編集を行ったものを2D用最終断面テーブルに登録する機能。 |
| TBL出力<br>（設備データ管理マスタテーブル→3D用最終断面テーブル） | 設備データ管理マスタテーブルから設備データを取得し、<br>必要に応じてデータの編集を行ったものを3D用最終断面テーブルに登録する機能。 |
| 認可情報登録 | 認可情報を登録する機能。 |
...
| ベクタデータ配信（可視化） | レイヤID、空間範囲などの条件をもとに、ベクタデータを3D用最終断面テーブルから取得し配信する機能。認可機能を含む機能。 |
